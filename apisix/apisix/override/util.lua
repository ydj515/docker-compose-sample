...
function _M.incoming(self, red, key, commit)
    ...
    if commit then
        -- local ttl = self.conf.redis_timeout or 60  -- TTL 기본값 60초
        local ttl = 60  -- TTL 기본값 60초

        local ok, err = red:set(excess_key, excess)
        if not ok then
            return nil, err
        end

        red:expire(excess_key, ttl)  -- TTL 설정
        ...
        red:expire(last_key, ttl)    -- TTL 설정
    end
